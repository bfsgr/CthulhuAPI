class Pick < ApplicationRecord
	...
	def empty_skills_with_pick_any
		errors.add(:skills, "shouldn't have skills since pick_any is true") if pick_any and not skills.empty?
	rescue ArgumentError
	end

	# check if there are enough skills to pick when pick_any is false
	def enough_skills_to_pick
		errors.add(:skills, 'not enough skills to pick') if not pick_any and skills.length <= number_of_picks
	rescue ArgumentError
	end
end
